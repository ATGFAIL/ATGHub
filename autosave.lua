...
-- ==================== UI LOADER ====================
local function LoadConfigSystem(Tabs, Library)
    ...
    local StreamerToggle = StreamerSection:AddToggle("StreamerModeToggle", {
        Title = "Protect Name",
        Description = "Protects your name while streaming",
        Default = false
    })

    StreamerToggle:OnChanged(function()
        if Library.Options.StreamerModeToggle.Value then
            StreamerMode:Start()
        else
            StreamerMode:Stop()
        end
    ...
    Library.Options.StreamerModeToggle:SetValue(false)
